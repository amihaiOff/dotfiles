...
function change_ec2_instance_type {
    ...
    if [[ -z "$INSTANCE_ID" ]]; then
        echo "ERROR: You must provide an instance ID with the -i parameter."
        usage
        return 1
    fi

    if [[ -z "$REQUESTED_TYPE" ]]; then
        echo "ERROR: You must provide an instance type with the -t parameter."
        usage
        return 1
    fi
    ...
    echo "    Requests type: $REQUESTED_TYPE"
    ...
    echo -n "Confirming that instance $INSTANCE_ID exists..."
    get_instance_info "$INSTANCE_ID"
    # If the instance doesn't exist, the function returns an error code <> 0.
    if [[ ${?} -ne 0 ]]; then
        echo "ERROR: I can't find the instance \"$INSTANCE_ID\" in the current AWS account."
        return 1
    fi
    ...
    echo "confirmed $INSTANCE_ID exists."
    echo "      Current type: $EXISTING_TYPE"
    echo "      Current state code: $EXISTING_STATE"

    # Are we trying to change the instance to the same type?
    if [[ "$EXISTING_TYPE" == "$REQUESTED_TYPE" ]]; then
        echo "ERROR: Can't change instance type to the same type: $REQUESTED_TYPE."
        return 1
    fi
    ...
    if [[ "$EXISTING_STATE" == "running" ]]; then
        ...
        if [[ $FORCE == false ]]; then
            ...
            echo "Forcing stop of instance without prompt because of -f."
        fi

        # stop the instance
        echo -n "Attempting to stop instance $INSTANCE_ID..."
        ...
        echo "Instance is not in running state, so not requesting a stop."
    fi;

    # Wait until stopped.
    echo "Waiting for $INSTANCE_ID to report 'stopped' state..."
    ...
    echo "Attempting to change type from $EXISTING_TYPE to $REQUESTED_TYPE..."
    RESPONSE=$(aws ec2 modify-instance-attribute \
                   --instance-id "$INSTANCE_ID" \
                   --instance-type "{\"Value\":\"$REQUESTED_TYPE\"}"
              )
    if [[ ${?} -ne 0 ]]; then
        echo "ERROR - AWS reports that it's unable to change the instance type for instance $INSTANCE_ID from $EXISTING_TYPE to $REQUESTED_TYPE.\n$RESPONSE"
        return 1
    fi
    echo "changed.\n"
    ...
    if [[ "$RESTART" == "true" ]]; then
        ...
        echo "Requesting to restart instance $INSTANCE_ID..."
        RESPONSE=$(aws ec2 start-instances \
                        --instance-ids "$INSTANCE_ID" \
                   )
        if [[ ${?} -ne 0 ]]; then
            echo "ERROR - AWS reports that it's unable to restart instance $INSTANCE_ID.\n$RESPONSE"
            return 1
        fi
        echo "started.\n"
        echo "Waiting for instance $INSTANCE_ID to report 'running' state..."
        ...
        echo "Restart was not requested with -r.\n"
    fi
}
